...
/*
* Make an MST and only print the start, end and connection citys
*/
void cheapest_path(Graph* self, int start, int end) {
	printf("\ Shortest path from %d,%d\n", start, end);
	int* min_cost_to_v = malloc(self->V);
	int* visited = malloc(self->V);
	int cost = 0;
	int count = 0;
	int start_place = start;
	//Inisitlises all min_cost and visted
	for (int i = 0; i < self->V; i++)
	{
		min_cost_to_v[i] = 100;
	...
	//Create new Graph "P" which will be the cheapest path
	Graph P;
	//Set the number of cities equal to the number of cities for param Graph
	P.V = self->V;
	//Inistlise empty edges for graph
			...
			if (visited[i] == 1)
			{
					...
					while (current != NULL) {
						if (visited[current->edge.to_vertex] == 666) {
							...
							//Set node to flag as visited 
							visited[current->edge.to_vertex] = 1;
							insert_at_front(&P.edges[i], new_node);
